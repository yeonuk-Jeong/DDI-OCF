...
def Test(dataset, Recmodel, epoch, w=None, multicore=0):
    ...
    with torch.no_grad():
        ...
        for batch_users in utils.minibatch(users, batch_size=u_batch_size):
            ...
            allPos = dataset.getUserPosItems(batch_users) # only train set edge
            groundTrue = [testDict[u] for u in batch_users] #only test set edge
            batch_users_gpu = torch.Tensor(batch_users).long()
            batch_users_gpu = batch_users_gpu.to(world.device)

            rating = Recmodel.getDrugsRating(batch_users_gpu)
            #rating = rating.cpu()
            exclude_index = []
            exclude_items = []
            ...
            rating[exclude_index, exclude_items] = -(1<<10) #Avoid recommending item(drugs) that already exist in the train set
